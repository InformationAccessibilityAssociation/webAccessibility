...
if(document.getElementById("lname").value == ""){
document.getElementById("errlist").innerHTML  = "<li role='alert'>你忘记输入姓！</li>";
return false;
}
if(document.getElementById("usrname").value == ""){
document.getElementById("errlist").innerHTML = "<li role='alert'>请输入登录名！</li>";
return false;
}
if(document.getElementById("pwd").value == ""){
document.getElementById("errlist").innerHTML = "<li role='alert'>请输入密码!</li>";
return false;
}
document.getElementById("errlist").innerHTML = "<li role='alert'>抱歉,不能登录：用户名或密码不匹配，请重试！</li>";
...
if (eFlag  > 0) {
$(document.getElementById("err_final")).text("登录失败：请修改错误重新输入！").focus();
return false; 
} 
$(document.getElementById("err_final")).html("登录失败：PIN / Email/Name匹配失败.请重试！").focus();
return false; 
...
// Suffix error text: 
$(errFld).parent().append('<span class="errtext">错误：错误数据！</span>');
return 1;
}
